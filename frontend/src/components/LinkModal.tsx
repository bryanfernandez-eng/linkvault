...
function LinkModal({ link, sections, onClose }: LinkModalProps) {
  ...
  useEffect(() => {
    if (link) {
      ...
      // Set default section to Uncategorized for new links
      const uncategorizedSection = sections.find(s => s.name === "Uncategorized")
      if (uncategorizedSection) {
        setValue("section_id", uncategorizedSection.id)
      }
    }
  }, [link, reset, sections, setValue])
      ...
      <div className="bg-slate-900/95 backdrop-blur-md border border-slate-700/50 rounded-lg shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">💡 Leave title empty to auto-fetch from website</p>
          </div>

          <div>
            <label htmlFor="title" className="block text-sm font-medium text-slate-300 mb-1">
              Title <span className="text-xs text-slate-400">(optional - will auto-fill)</span>
            </label>
            <input
              id="title"
              type="text"
          ...
          <div>
            <label htmlFor="description" className="block text-sm font-medium text-slate-300 mb-1">
              Description <span className="text-xs text-slate-400">(optional - will auto-fill)</span>
            </label>
            <textarea
              id="description"
